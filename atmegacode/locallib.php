...
class assign_submission_atmegacode extends assign_submission_plugin {
    ...
    public function save(stdClass $submission, stdClass $data) {
            ...
            if ($zipper->open($tmp) === true) {
                ...
                foreach ($grouped as $folder => $types) {
                    ...
                    foreach ($types['c'] ?? [] as $name => $time) {
                        if (in_array($name, $ignorec, true)) {
                            continue;
                        }
                        $hashex = !empty($types['hex'][$name]);
                        $haself = !empty($types['elf'][$name]);
                        if (!$hashex && !$haself) {
                            $errors[] = get_string(
                                'error_nohexfile',
                                'assignsubmission_atmegacode',
                                (object)[
                                    'folder' => $folder,
                                    'file' => preg_replace('/\.c$/', '', $name) . '.c',
                                ]
                            );
                        } else if ($time > $types['hex'][$name]) {
                ...
                // Warn if HEX is much newer than C/H (over 1 hour).
                foreach ($grouped as $folder => $types) {
                    if (!isset($types['hex'])) {
                        continue;
                    }
                    foreach ($types['hex'] as $name => $hexmtime) {
                        $maxsource = 0;
                        if (!empty($types['c'][$name])) {
                            $maxsource = max($maxsource, $types['c'][$name]);
                        }
                        if (!empty($types['h'][$name])) {
                            $maxsource = max($maxsource, $types['h'][$name]);
                        }
                        // If there was any source file, and hex is > 1 hour newer.
                        if ($maxsource > 0 && ($hexmtime - $maxsource) > 3600) {
                            $hexid[] = get_string('warning_hex_newer', 'assignsubmission_atmegacode', (object)[
                                'file' => $name . '.hex',
                                'folder' => $folder,
                            ]);
                        }
        ...
        if ($ex = $DB->get_record('atmega_flags', ['submissionid' => $submission->id])) {
            $flag->id = $ex->id;
            $DB->update_record('atmega_flags', $flag);
        } else {
