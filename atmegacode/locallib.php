                ...
                if ($zip->open($tmpzip) === true) {
                    ...
                    for ($i = 0; $i < $zip->numFiles; $i++) {
                        ...
                        if (in_array($entryext, ['c', 'h', 'hex'])) {
                            $folderfiles[$folder][$entryext][] = $filetime;
                        }
                    ...
                    foreach ($folderfiles as $folder => $types) {
                        if ((!empty($types['c']) || !empty($types['h'])) && !empty($types['hex'])) {
                            $max_src_time = 0;
                            if (!empty($types['c'])) $max_src_time = max($types['c']);
                            if (!empty($types['h'])) $max_src_time = max($max_src_time, max($types['h']));
                            $min_hex_time = min($types['hex']);
                            if ($max_src_time > $min_hex_time) {
                                $errors[] = "Code in the folder <strong>$folder</strong> was possibly not re-compiled?! This must be amended.";
                            }
                        }
                    }

                    foreach ($folderfiles as $folder => $types) {
                    // Skip the root folder if you don't want to enforce this for root
                    if ($folder === 'root') continue;
                    // If folder contains any .c or .h but NO .hex file
                    if ((isset($types['c']) || isset($types['h'])) && empty($types['hex'])) {
                        $errors[] = "Folder <strong>$folder</strong> contains no hex file. Please recompile or amend your submission.";
                    }
                    }
