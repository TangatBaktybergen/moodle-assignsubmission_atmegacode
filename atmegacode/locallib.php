...
class assign_submission_atmegacode extends assign_submission_plugin {
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        if(count($files)!==2||!$pdf||!$zip){
            $SESSION->atmegacode_error = get_string('error_filecount','assignsubmission_atmegacode',(object)['pdf'=>$expectedpdf,'zip'=>$expectedzip]);
            redirect(new moodle_url('/mod/assign/view.php',['id'=>$this->assignment->get_course_module()->id]));
        }
            ...
            if($zipper->open($tmp)===true){
                ...
                if(count($nested)>0){
                    foreach($nested as $p=>$subs){
                        $errors[] = get_string('error_nestedfolders','assignsubmission_atmegacode',(object)['folder'=>$p,'subfolders'=>implode(', ',$subs)]);
                    }
                }
                ...
                for($i=0;$i<$zipper->numFiles;$i++){
                    ...
                    if ($size === 0) {
                        if ($ext === 'hex') {
                            $errors[] = get_string('error_hexempty',   'assignsubmission_atmegacode', $e);
                        } else {
                    ...
                    if($ext==='zip'){
                        $errors[]=get_string('error_nestedzip','assignsubmission_atmegacode',$e);
                        continue;
                    }
                ...
                foreach($grouped as $folder=>$types){
                    ...
                    foreach($types['c'] ?? [] as $name=>$time){
                        ...
                        if(empty($types['hex'][$name])){
                            ...
                            $errors[]=get_string('error_timestamp_c','assignsubmission_atmegacode',$name);
                        }
                    }
                    foreach($types['h'] ?? [] as $name=>$time){
                        if(in_array($name,$ignorec,true)) continue;
                        if(!empty($types['hex'][$name]) && $time > $types['hex'][$name]){
                            $errors[]=get_string('error_timestamp_h','assignsubmission_atmegacode',$name);
                        }
                    }
                }
                ...
                for($i=0;$i<$zipper->numFiles;$i++){
                    ...
                    if($stream){
                        ...
                        $idhex = ''; foreach(str_split($studentid) as $c){ $idhex .= sprintf('%02X',ord($c)); }
                        ...
                        if(strpos($datahex,$idhex) === false){
                            ...
                            for($j=0;$j<strlen($datahex)-9;$j+=2){
                                ...
                                $asc = ''; for($k=0;$k<10;$k+=2){ $ch=chr(hexdec(substr($substr,$k,2))); if($ch<'0'||$ch>'9'){ $asc=''; break;} $asc.=$ch; }
                                if($asc && preg_match('/^\d{5}$/',$asc)){ $found=$asc; break; }
                            }
                            $fld = dirname($e); if($fld==='.'||$fld==='\\') $fld='root';
                            $hexid[] = get_string('error_idmismatch','assignsubmission_atmegacode',(object)[
                                'folder'=>$fld, 'file'=>basename($e), 'expected'=>$studentid, 'found'=>$found?:get_string('none','assignsubmission_atmegacode')
                            ]);
                        }
                    }
                }
        ...
        foreach($hexun as $f){ $errors[] = get_string('error_hexuncompiled','assignsubmission_atmegacode',$f); }

        // Abort on errors.
        if(!empty($errors)){
            $html='<ul>';
            foreach($errors as $err){ $html.='<li>'.$err.'</li>'; }
            $html.='</ul>';
 ...
 */
    public function view_summary(stdClass $submission, &$showviewlink) {
        ...
        if (!empty($SESSION->atmegacode_error)) {
            ...
            get_string('error_feedback_title', 'assignsubmission_atmegacode')
            ) . html_writer::empty_tag('br') . $SESSION->atmegacode_error;
        ...
        // Allow Moodle to show the file listing in expanded view.
         $showviewlink = true;
