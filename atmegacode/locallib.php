...
class assign_submission_atmegacode extends assign_submission_plugin {
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        if ($ex = $DB->get_record('assignsubmission_at', ['submissionid' => $submission->id])) {
            $flag->id = $ex->id;
            $DB->update_record('assignsubmission_at', $flag);
        } else {
            $DB->insert_record('assignsubmission_at', $flag);
        }
        // Success.
        $cache = cache::make('assignsubmission_atmegacode', 'sessionmessages');
        $cache->delete('error');
    ...
    public function view(stdClass $submission) {
        ...
        if (has_capability('mod/assign:grade', $this->assignment->get_context())) {
            ...
            $record = $DB->get_record('assignsubmission_at', ['submissionid' => $submission->id]);
