...
class assign_submission_atmegacode extends assign_submission_plugin {
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        foreach ($files as $file) {
            ...
            if ($zipper->open($tmp) === true) {
                ...
                foreach ($grouped as $folder => $types) {
                    ...
                    // New HEX checks: presence and timestamp comparison.
                    if (empty($types['hex'])) {
        ...
        // Trigger Moodle event to log submission.
        \mod_assign\event\assessable_uploaded::create([
            'context' => $this->assignment->get_context(),
            'objectid' => $submission->id,
            'relateduserid' => $USER->id,
            'other' => [
                'content' => 'ATmega code submission (ZIP + PDF)',
                'pathnamehashes' => [],
            ],
        ])->trigger();
        // Success.
